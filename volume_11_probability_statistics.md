...
### 10.1 Naive Bayes — 베이즈 정리의 직접 응용

스팸 필터·문서 분류의 고전적 모델입니다. *각 특성이 조건부 독립* 이라 가정하고:
...
texts = ["free money now", "meeting at noon", "cheap pills"]
labels = [1, 0, 1]   # 1 = spam

vec = CountVectorizer()
X = vec.fit_transform(texts)
clf = MultinomialNB().fit(X, labels)
print(clf.predict(vec.transform(["free pills"])))   # [1]
```

이 단순한 모델이 *2000 년대 초 스팸 필터의 표준* 이었고, 지금도 *작은 분류 + 빠른 추론* 이 필요한 상황에서 사용됩니다.

### 10.2 베이지안 신경망

표준 신경망은 가중치를 *점 추정 (Point Estimate)* 합니다. 베이지안 신경망은 *가중치 자체를 분포로* 다룹니다.

$$P(W \mid D) \propto P(D \mid W) P(W)$$

추론 시 여러 W 샘플을 사용해 *불확실성 정량화* 가 가능합니다. 의료·금융 같은 *결정의 신뢰도가 중요한* 영역에서 활용.

### 10.3 디퓨전 모델의 베이즈 시각

디퓨전 모델 (Vol 80) 은 *노이즈 추가/제거* 의 두 마르코프 체인이며, *역방향 분포* 를 학습합니다. 베이즈 사고가 모델 설계에 깊이 들어가 있습니다.

### 10.4 챕터 정리

베이즈 정리는 Naive Bayes 같은 *고전 ML* 부터 *베이지안 신경망·디퓨전 모델* 같은 현대 모델까지 광범위하게 사용됩니다.
...
100 개 샘플로 *95% 정확도* 를 봤다고 모델이 95% 라 단정할 수 없습니다. *95% CI 가 약 ±5%p* 이므로 *90-100%* 의 범위.

샘플 수에 따른 신뢰구간 (정확도 95% 기준):
...
작은 샘플의 결과는 *항상 신뢰구간과 함께* 보고합니다.

### 11.2 다중 비교 (Multiple Comparison)

100 개 모델을 비교해 *가장 좋은 1 개* 를 골랐다면, 그 1 개가 *우연히* 가장 좋게 나왔을 가능성이 큽니다. *Bonferroni correction* 같은 보정이 필요.

### 11.3 데이터 누수 (Data Leakage)

학습 데이터에 *미래 정보* 또는 *테스트 데이터의 특성* 이 섞여 들어가면, 평가 결과가 비현실적으로 좋아집니다. *시간 순서·해시 기반* 분할로 방지.

### 11.4 클래스 불균형

99% 가 음성, 1% 가 양성인 데이터에서 *모두 음성으로 예측* 하면 정확도 99%. 정확도가 무의미. F1·AUC·Recall 같은 *불균형에 강한 메트릭* 사용.

### 11.5 챕터 정리

작은 샘플·다중 비교·데이터 누수·클래스 불균형의 4 함정은 *통계적 사고* 가 약한 사람이 가장 자주 빠지는 곳입니다. 모든 결과를 *신뢰구간·메트릭 적합성* 함께 보고하는 습관이 방어책.
...
다음 권은 [Volume 12 — 정보 이론](./volume_12_information_theory.md) 입니다. 거기서는 *교차엔트로피와 KL 발산이 왜 그렇게 자주 등장하는가* 를 정보 이론으로 답합니다.
...
[Volume 12 — 정보 이론](./volume_12_information_theory.md)
