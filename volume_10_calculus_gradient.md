...
기본 경사하강법은 다음 변형들을 가집니다 (Vol 33 에서 자세히):
...
자세한 학습률 스케줄은 Vol 40 (학습률 스케줄 깊이) 에서 다룹니다.
...
## 9. 옵티마이저 변형 — SGD 에서 Adam 까지

### 9.1 SGD 의 한계

기본 경사하강법은 *항상 그래디언트 방향* 으로만 이동합니다. 이는 *골짜기 모양 손실* 에서 *지그재그 진동* 을 만들어 수렴을 느리게 합니다.

```python
import numpy as np

# 좁은 타원형 골짜기
def loss(W): return W[0]**2 + 100 * W[1]**2
def grad(W): return np.array([2*W[0], 200*W[1]])

W = np.array([5.0, 5.0])
lr = 0.01
for _ in range(50):
    W = W - lr * grad(W)
print(W)   # 첫 차원은 진동, 둘째는 빠르게 수렴
```

### 9.2 모멘텀

*모멘텀* 은 이전 이동 방향을 *관성처럼* 누적합니다.

$$v_{t+1} = \mu v_t + \nabla L(W_t), \quad W_{t+1} = W_t - \eta v_{t+1}$$

$\mu$ (momentum coefficient) 가 0.9-0.99 로 설정되며, 진동을 평균화해 *지그재그를 줄이고 수렴 속도를 올립니다*.
...
- *1차 모멘트* (그래디언트의 이동 평균) — 모멘텀과 같음
- *2차 모멘트* (그래디언트 제곱의 이동 평균) — 좌표별 적응 학습률

```python
W = np.array([5.0, 5.0])
m = np.zeros_like(W)  # 1차 모멘트
v = np.zeros_like(W)  # 2차 모멘트
...
각 좌표가 *자기에게 맞는 학습률* 을 가지므로 좁은 골짜기에서도 안정적으로 수렴합니다.

### 9.4 AdamW

표준 Adam 의 *L2 정칙화* 에는 미묘한 결함이 있어 (가중치 감쇠가 학습률에 의존), AdamW 는 *Weight Decay 를 분리* 합니다.

```python
# AdamW 의 핵심 차이: weight decay 를 그래디언트 외부로
W = W - lr * (m_hat / (np.sqrt(v_hat) + eps) + wd * W)
```

`[VERIFY: Loshchilov & Hutter (2017) Decoupled Weight Decay Regularization]`. 현재 거의 모든 트랜스포머 학습은 AdamW 를 표준으로 씁니다.

### 9.5 챕터 정리

옵티마이저 진화는 *SGD → 모멘텀 → Adam → AdamW* 로 이어졌으며, 각 단계가 이전의 한계를 해결했습니다. 모두 *그래디언트 반대 방향으로 이동* 한다는 핵심은 같으며, *어떻게 누적·적응시키는가* 만 다릅니다.
...
학습 도중 손실이 갑자기 NaN 으로 바뀌는 일이 있습니다. 흔한 원인:

- *학습률 과다* — 그래디언트가 폭발해 가중치가 무한대로
- *수치 안정 안 된 연산* — `log(0)` 또는 `1/0`
- *입력에 NaN/Inf* — 데이터 전처리 누락
- *softmax 의 overflow* — `exp(큰 수)` 가 무한대
- *float16 의 표현 한계* — 작은 그래디언트가 0 으로 round
...
그래디언트의 *L2 노름이 `max_norm` 을 넘으면 비례 축소* 합니다. 트랜스포머·RNN 학습에서 거의 필수.

### 10.4 학습률 워밍업

큰 모델은 *처음 몇 백 스텝* 동안 학습률을 *0 에서 목표 값까지 선형 증가* 시킵니다. 초기 그래디언트의 *큰 분산* 이 모델을 망치지 않도록 보호.
...
학습 안정성은 *NaN 진단·그래디언트 클리핑·워밍업* 의 세 도구로 대부분 확보됩니다. 큰 모델일수록 이 도구들의 비중이 큽니다.
...
다음 권은 [Volume 11 — 확률과 통계](./volume_11_probability_statistics.md) 입니다. 거기서는 *모든 ML 손실 함수가 확률 분포에서 유도된다* 는 사실을 이해합니다.
...
[Volume 11 — 확률과 통계](./volume_11_probability_statistics.md)
